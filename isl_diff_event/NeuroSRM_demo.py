...
# 为什么这里使用双曝光跨度，而不是只使用第一张的 200000 us：
#   1. CMax 是纯事件运动估计。更长窗口提供更多事件，生成的分段事件帧和 IWE
#      信噪比更高，12 段轨迹中的每一段也能获得更多约束。
#   2. 更长的时间基线会积累更明显的像素位移，使很小的亚像素运动更容易通过
#      配准和 contrast maximization 观测出来。
#   3. 一条轨迹同时覆盖相邻两次 APS 曝光，后续可以在这段时间内选择不同参考
#      时刻生成 IWE，而不必为每张 frame 独立估计一条可能不连续的轨迹。
# 但要注意：双曝光跨度只是用来增强事件轨迹估计。当前最终 frame loss 仍只使用
# frames_mov（第 frame_ind 张 APS），其运动模糊 PSF 也只在首个 win_frame=200000 us
# 内积分；第二张 APS 并没有直接加入最终图像监督。该选择还隐含假设：物体和相机
# 在 418341 us 内可由同一条 12 段连续轨迹描述。若运动突变或场景非刚性变化明显，
# 较长窗口反而会使事件无法聚焦，此时应关闭 compare_2frame 或缩短窗口。
vars = []
