...
def compute_dL_vec(uni_flow_np_y, uni_flow_np_x, L):
    height, width = L.shape
    fy = cv2.resize(uni_flow_np_y,(height, width ))
    fx = cv2.resize(uni_flow_np_x,(height, width ))
    A_ = computeMatrixA(fy,fx).toarray()
    x = L.ravel()
    dL = np.dot(A_, x)
    dL = dL.reshape(height, width)
    return dL

def image_reconstruction(flow_y, flow_x, iwe, reg_type, reg_weight,itr_ind = 40,img_init=None):
    ...
    uDop = [pylops.FirstDerivative(dims=(height, width), axis=0, edge=False),
            pylops.FirstDerivative(dims=(height, width), axis=1, edge=False),
            pylops.Laplacian(dims=(height, width), edge=True, dtype=np.float64)]
    A_ = computeMatrixA(flow_y, flow_x)
    Op = pylops.MatrixMult(A_)
    iwe = iwe / (np.amax(np.abs(iwe))*6)
    y_k = iwe.ravel()
    if img_init is None:
        img_init = np.ones(height*width)/ 2.0
    else:
        img_init = img_init.ravel()
    #print(img_init.shape)
    if reg_type == "l1":
        x_best = pylops.optimization.sparsity.splitbregman(Op, y_k, Dop, niter_outer=itr_ind,
                                                           niter_inner=5, mu=1.0,
                                                           epsRL1s=[reg_weight[0], reg_weight[1]], tol=1e-4,
                                                           tau=1., show=True, x0=img_init,
                                                           **dict(iter_lim=10, damp=1e-4))[0]
    elif reg_type == "l2":
        x_best = pylops.optimization.leastsquares.regularized_inversion(Op, y_k, D2op, epsRs=[reg_weight], x0=img_init,
                                                                        **dict(iter_lim=itr_ind, show=True))[0]
    elif reg_type == "uL1":
        x_best = pylops.optimization.sparsity.splitbregman(Op, y_k, RegsL1=uDop, niter_outer=itr_ind,
                                                           niter_inner=5, mu=1.0,
                                                           epsRL1s=[reg_weight[0], reg_weight[1],reg_weight[2]], tol=1e-4,
                                                           tau=1., show=True, x0=img_init,
                                                           **dict(iter_lim=10, damp=1e-4))[0]

    elif reg_type == "l1l2":
        x_best = pylops.optimization.sparsity.splitbregman(Op, y_k, RegsL1 =Dop, niter_outer=itr_ind,
                                                           niter_inner=5, mu=1.0,RegsL2 = D2op,
                                                           epsRL1s=[reg_weight[0], reg_weight[1]], tol=1e-4,epsRL2s=[reg_weight[2]],
                                                           tau=1., show=True, x0=img_init,
                                                           **dict(iter_lim=10, damp=1e-4))[0]
    # tol = 1e-4,
    # tau = 1.,
    return x_best.reshape(height, width) #minmax_norm(x_best.reshape(height, width))


def image_reconstruction_2d(flow_y, flow_x, iwe, reg_type, reg_weight, itr_ind=40):
    height, width = iwe.shape
    # Build a 9-dignoal matrix
    A_ = np.vstack[flow_y, flow_x] #computeMatrixA(flow_y, flow_x)
    ...
    if reg_type == "l1":
        ...
        x_best = pylops.optimization.sparsity.splitbregman(Op, y_k, Dop, niter_outer=itr_ind,
                                                           niter_inner=5, mu=1.0,
                                                           epsRL1s=[reg_weight[0], reg_weight[0]],
                                                           epsRL2s=[reg_weight[1], reg_weight[1]], show=True,
                                                           x0=img_init,
                                                           **dict(iter_lim=10, damp=1e-4))[0]
    # tol = 1e-4,
    # tau = 1.,
...
def image_rec_cnn(flow_y, flow_x, iwe, device, model_path, weight1, weight2, x_best = None,grad_des_iters = 200, x8 = True):
    n_channels = 1
    ...
    if x_best is None:
        x_best = pylops.optimization.sparsity.splitbregman(Op, y_k, Dop, niter_outer=10,
                                                           niter_inner=5, mu=1.0, epsRL1s=[5e-2, 5e-2],
...
from utils.utility import lin_log

###Todo: need down sampling
def simple_image_rec_cnn(flow_y, flow_x, iwe, device, model_path, weight1, weight2, weight3, grad_des_iters = 200, x_best = None, frame = None, filter= None):
    n_channels = 1
    model = net(in_nc=n_channels+1, out_nc=n_channels, nc=[64, 128, 256, 512], nb=4, act_mode='R', downsample_mode="strideconv", upsample_mode="convtranspose")
    model.load_state_dict(torch.load(os.path.join(model_path, 'drunet_gray.pth')), strict=True)
    model.eval()
    ...
    img_init = x_best.reshape(1, 1, height, width)
    img_iwe_ = img_iwe_.reshape(1, 1, height, width)
    frame = frame.reshape(1, 1, height, width)
    f_torch = torch.from_numpy(np.ascontiguousarray(frame)).float().to(device)
    x = torch.from_numpy(np.ascontiguousarray(img_init)).float().to(device)
    z = x.clone()
    y = torch.from_numpy(np.ascontiguousarray(img_iwe_)).float().to(device)

    rhos_np, sigmas_np = utils_model.get_rho_sigma(sigma=max(0.255/255., noise_level_img) * weight2, iter_num=iter_num, modelSigma1=modelSigma1, modelSigma2=modelSigma2, w=1.0)
    rhos, sigmas = torch.tensor(rhos_np).to(device), torch.tensor(sigmas_np).to(device)   ## good point is that the pnp is independent of the data itself, should have great inplefication
    ...
    for i in range(iter_num):
        ...
        for it in range(grad_des_iters):
            optimizer.zero_grad()
            warped_x = F.grid_sample(x, grid_pos, mode="bilinear", padding_mode="zeros", align_corners = True)
            lx_blur =filter(torch.exp(x)).squeeze()
            output = loss(y, (x - warped_x)) + rhos[i].float() * loss(x, z)   + weight3 * loss(lx_blur, f_torch)
            try:
                output.backward()
            except Exception as e:
                print(e)
...
def minmax_norm(x,q = 99,a_min = 1e-2):
    den = np.percentile(x, q) - np.percentile(x, 100-q)
    if den != 0:
        x = (x - np.percentile(x, 100-q)) / den
    return np.clip(x, a_min , 1)

def compute_dL_pylops(uni_flow_np_y, uni_flow_np_x, L,kind ="forward"):
    height, width = L.shape
    Dop = [pylops.FirstDerivative(dims=(height, width),kind = kind, axis=0, edge=False),
           pylops.FirstDerivative(dims=(height, width),kind = kind, axis=1, edge=False)]
    gy = Dop[0] *L
    gx = Dop[1] * L
...
class ImageReconstructor(object):
    def __init__(self, flow_torch,iwe_torch=None):
        """
    ...
    def _check_iwe(self, iwe):
        ...
        self.iwe = iwe * self.border_mask / self.flow_np_mag



    
    def image_rec_from_events_l1(self, events_torch, reg_weight=1e-1,itr_ind = 40):
        """
        ...
        iwe = iwe.cpu().numpy().reshape(self.height, self.width)
        self.iwe = iwe * self.border_mask / self.flow_np_mag
        img_rec = image_reconstruction(self.uni_flow_np_y, self.uni_flow_np_x, self.iwe, "l1", reg_weight,itr_ind)
        return img_rec
    ...
    def image_rec_from_iwe_l1(self, iwe_torch, reg_weight=1e-1,itr_ind = 40,img_init= None):
        """
        ...
        iwe = iwe * self.border_mask / self.flow_np_mag
        img_rec = image_reconstruction(self.uni_flow_np_y, self.uni_flow_np_x, iwe, "l1", reg_weight,itr_ind,img_init)
        return img_rec

    def image_rec_from_iwe_l2(self, iwe_torch, reg_weight=3e-1,itr_ind = 200,img_init= None):
        """
        iwe_torch: (torch.Tensor) [batch_size x 1 x H x W] image of warped events(with polarity)
        reg_weight: (float) regularization weight
        """
        ...
        iwe = iwe_torch.cpu().numpy().reshape(self.height, self.width)
        iwe = iwe * self.border_mask / self.flow_np_mag
        img_rec = image_reconstruction(self.uni_flow_np_y, self.uni_flow_np_x, iwe, "l2", reg_weight,itr_ind,img_init)
        return img_rec

    def image_rec_from_iwe_l1l2(self, iwe_torch, reg_weight=[1e-1, 3e-1],itr_ind = 40,img_init= None):
        """
        iwe_torch: (torch.Tensor) [batch_size x 1 x H x W] image of warped events(with polarity)
        reg_weight: (float) regularization weight
        """
        ...
        iwe = iwe_torch.cpu().numpy().reshape(self.height, self.width)
        iwe = iwe * self.border_mask / self.flow_np_mag
        img_rec = image_reconstruction(self.uni_flow_np_y, self.uni_flow_np_x, iwe, "l1l2", reg_weight,itr_ind,img_init)
        return img_rec
    def image_rec_from_iwe_ul1(self, iwe_torch, reg_weight=[1e-1, 1e-1, 1e-1],itr_ind = 40,img_init= None):
        """
        iwe_torch: (torch.Tensor) [batch_size x 1 x H x W] image of warped events(with polarity)
        reg_weight: (float) regularization weight
        """
        ...
        iwe = iwe_torch.cpu().numpy().reshape(self.height, self.width)
        iwe = iwe * self.border_mask / self.flow_np_mag
        img_rec = image_reconstruction(self.uni_flow_np_y, self.uni_flow_np_x, iwe, "uL1", reg_weight,itr_ind,img_init)
        return img_rec


    def image_rec_from_iwe_cnn(self, iwe_torch,cnn_model_path="denoiser_model", weight1=2.5, weight2=1.3,x_best= None,grad_des_iters = 200,x8 = True):
        """
        iwe_torch: (torch.Tensor) [batch_size x 1 x H x W] image of warped events(with polarity)
        cnn_model_path: (string) path to the denoiser model
        weight1, weight1: (float) weights to control the regularzation, for details see: https://github.com/cszn/DPIR
        ...
        iwe = iwe_torch.cpu().numpy().reshape(self.height, self.width)
        iwe = iwe * self.border_mask / self.flow_np_mag
        img_rec = image_rec_cnn(self.uni_flow_np_y, self.uni_flow_np_x, iwe, self.device, cnn_model_path, weight1, weight2, x_best,grad_des_iters,x8)
        return img_rec

    def image_rec_from_NeuroSRM_cnn(self, iwe_torch, cnn_model_path="denoiser_model", weight1=2.5, weight2=1.3,  weight3 = 0.5,grad_des_iters=200, x_best=None, frame=None, filter=None):
        """
        events: (torch.Tensor) [batch_size x N x 4] input events (ts, x, y, p), p should either be 0 or 1
        cnn_model_path: (string) path to the denoiser model
        weight1, weight1: (float) weights to control the regularzation, for details see: https://github.com/cszn/DPIR
        ...
        iwe = iwe_torch.cpu().numpy().reshape(self.height, self.width)
        iwe = iwe * self.border_mask / self.flow_np_mag
        img_rec = simple_image_rec_cnn(self.uni_flow_np_y, self.uni_flow_np_x, iwe, self.device, cnn_model_path, weight1, weight2, weight3,grad_des_iters,
                             x_best, frame, filter)
        return img_rec
