...
class Zern(ABC):
    ...
    def __init__(self, n, normalise=NORM_NOLL):
        ...
        ntab = np.zeros(nk, dtype=np.int_)
        mtab = np.zeros(nk, dtype=np.int_)
    ...
    def matrix(self, Phi):
        ...
        elif self.shape[0]*self.shape[1] != Phi.shape[0]:
            raise ValueError(f'Phi.shape should be {self.shape}')

        # return Phi.reshape(self.shape, order='F')
        # return Phi.T.reshape(self.shape[::-1]).T

        return Phi.reshape(self.shape[::-1])
    ...
    def eval_grid(self, a, matrix=False,device ="cpu"):
        ...
        if a.shape[0] != self.nk:
            raise ValueError(f'a.size = {a.shape[0]} but self.nk = {self.nk}')
        Phi = torch.matmul(torch.from_numpy(self.ZZ).to(device), a)
        if matrix:
    ...
    def save_h5py(self, f, prepend=None, params=HDF5_options):
        ...
        f.create_dataset(prefix + 'n', data=np.array([self.n], dtype=np.int_))

        f.create_dataset(prefix + 'nk', data=np.array([self.nk], dtype=np.int_))

        f.create_dataset(
            prefix + 'normalise',
            data=np.array([self.normalise], dtype=np.int_))
...
class FitZern:
    ...
    def save_h5py(self, f, prepend=None, params=HDF5_options):
        ...
        f.create_dataset(prefix + 'K', data=np.array([self.K], dtype=np.int_))

        f.create_dataset(prefix + 'L', data=np.array([self.L], dtype=np.int_))
